...
Then customize the API client in `api.py` to connect to your actual device or service.
...
- **`script/check`** - Run both linting and type checking (useful before commits)
...
├── api.py              # API client for your device/service
...
The [`config_flow.py`](custom_components/ha_integration_domain/config_flow.py) file handles user setup through the UI. Users can:
...
- Reconfiguration support via `async_step_reconfigure`
...
**[`sensor.py`](custom_components/ha_integration_domain/sensor.py)**

- Shows how to create sensors with state values
- Demonstrates state classes and device classes
- Example: Temperature, battery level, status text

**[`binary_sensor.py`](custom_components/ha_integration_domain/binary_sensor.py)**

- Shows binary (on/off) sensors
- Uses device classes for proper icons
- Example: Motion detected, door open, connectivity status

**[`switch.py`](custom_components/ha_integration_domain/switch.py)**
...
Each platform shows best practices for entity setup, naming, and data handling.

### API client

The [`api.py`](custom_components/ha_integration_domain/api.py) file provides an async API client:
...
Replace the dummy API calls with your actual device/service API.
